...
<head>
    ...
    <link href="/css/jcldetail.css" rel="stylesheet" type="text/css">
...
<body>
    ...
    <c:forEach var="n" items="${list}">
        <form action="jclUpdateMerge?jcl_id=${n.id}&jcl_num=${n.num}&jcl_charger=${n.charger}" method="post" >
            ...
            <div id="btn-block"> 
                ...
                <input class="btn4" type="submit" value="업데이트" onclick="return confirm('업데이트하시겠습니까?');" >
            </div>
...
<!-- <script>
				function updateJclDetail() {
					
					const a = document.getElementById(inputJobclass);
					console.dir(a);
					
					const result = confirm('수정하시겠습니까?');
					
					if(result == true){
						if(document.getElementById(inputJobclass).value != '온라인공통' || 
						document.getElementById(inputJobclass).value != '종합정보' || 
						document.getElementById(inputJobclass).value != '카드' || 
						document.getElementById(inputJobclass).value != '여신' || 
						document.getElementById(inputJobclass).value != '수신' || 
						document.getElementById(inputJobclass).value != '대행/단위' || 
						document.getElementById(inputJobclass).value != '스마트뱅킹' || 
						document.getElementById(inputJobclass).value != '웹뱅킹' || 
						document.getElementById(inputJobclass).value != '채널') {
							alert("업무분류 정보가 잘못되었습니다.");
						} else if(document.getElementById(inputFactors).value != '외부기관' || 
						document.getElementById(inputFactors).value != '프로그램' || 
						document.getElementById(inputFactors).value != '시스템' || 
						document.getElementById(inputFactors).value != '본부/영업점'){
							alert("오류요인 정보가 잘못되었습니다.");
						}
